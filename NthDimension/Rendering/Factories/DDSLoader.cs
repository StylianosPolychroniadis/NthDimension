...
#region Private Members
        ...
        {
            ApplicationBase.Instance.Renderer.GetError();
...
#region Translate Header to less cryptic representation
                ...
                // start by checking if all forced flags are present. Flags indicate valid fields, but aren't written by every FirstPersonViewController .....
...
#region Generate and Bind Handle
                ApplicationBase.Instance.Renderer.GenTextures(1, out texturehandle);
                ApplicationBase.Instance.Renderer.BindTexture(dimension, texturehandle);
...
#region Create TexImage
...
#if OPENTK3
                                    GCHandle texture2D_IntPtr = GCHandle.Alloc(RawDataOfSurface, GCHandleType.Pinned);
                                    Game.Instance.Renderer.CompressedTexImage2D(TextureTarget.Texture2D,
                                                             Level,
                                                             ...
                                                             SurfaceSizeInBytes,
                                                             texture2D_IntPtr.AddrOfPinnedObject());
                                    texture2D_IntPtr.Free();
#else
                                    ApplicationBase.Instance.Renderer.CompressedTexImage2D(TextureTarget.Texture2D,
                                        Level,
                                        _PixelInternalFormat,
                                        Width,
                                        Height,
...
#if OPENTK3
                                    GCHandle textureCubeMap_IntPtr = GCHandle.Alloc(RawDataOfSurface, GCHandleType.Pinned);
                                    Game.Instance.Renderer.CompressedTexImage2D(TextureTarget.TextureCubeMapPositiveX + Slices,
                                                             Level,
                                                             _PixelInternalFormat,
                                                             Width,
                                                             Height,
                                                             TextureLoaderParameters.Border,
                                                             SurfaceSizeInBytes,
                                                             textureCubeMap_IntPtr.AddrOfPinnedObject());
                                    textureCubeMap_IntPtr.Free();
#else
                                    ApplicationBase.Instance.Renderer.CompressedTexImage2D(TextureTarget.TextureCubeMapPositiveX + Slices,
                                        Level,
                                        _PixelInternalFormat,
                                        Width,
                                        Height,
                            ...
                            ApplicationBase.Instance.Renderer.Finish();
...
#region Query Success
                            ...
                            switch (dimension)
                            {
                                ...
                                case TextureTarget.Texture3D:
                                    ApplicationBase.Instance.Renderer.GetTexLevelParameter(dimension, Level, GetTextureParameter.TextureWidth, out width);
                                    ApplicationBase.Instance.Renderer.GetTexLevelParameter(dimension, Level, GetTextureParameter.TextureHeight, out height);
                                    ApplicationBase.Instance.Renderer.GetTexLevelParameter(dimension, Level, GetTextureParameter.TextureInternalFormat, out internalformat);
                                    ApplicationBase.Instance.Renderer.GetTexLevelParameter(dimension, Level, GetTextureParameter.TextureCompressed, out compressed);
                                    break;
                                case TextureTarget.TextureCubeMap:
                                    ApplicationBase.Instance.Renderer.GetTexLevelParameter(TextureTarget.TextureCubeMapPositiveX + Slices, Level, GetTextureParameter.TextureWidth, out width);
                                    ApplicationBase.Instance.Renderer.GetTexLevelParameter(TextureTarget.TextureCubeMapPositiveX + Slices, Level, GetTextureParameter.TextureHeight, out height);
                                    ApplicationBase.Instance.Renderer.GetTexLevelParameter(TextureTarget.TextureCubeMapPositiveX + Slices, Level, GetTextureParameter.TextureInternalFormat, out internalformat);
                                    ApplicationBase.Instance.Renderer.GetTexLevelParameter(TextureTarget.TextureCubeMapPositiveX + Slices, Level, GetTextureParameter.TextureCompressed, out compressed);
                                    break;
                                default:
                                    throw Unfinished;
                            }
                            GLError = ApplicationBase.Instance.Renderer.GetError();
                            ...
                            {
                                ApplicationBase.Instance.Renderer.DeleteTextures(1, ref texturehandle);
                                throw new ArgumentException("ERROR: Something went wrong after Game.Instance.Renderer.CompressedTexImage(); Last GL Error: " + GLError.ToString());
                            }
...
#region Set States properly
                    ApplicationBase.Instance.Renderer.TexParameter(dimension, TextureParameterName.TextureBaseLevel, 0);
                    ApplicationBase.Instance.Renderer.TexParameter(dimension, TextureParameterName.TextureMaxLevel, trueMipMapCount);

                    int TexMaxLevel;
                    ApplicationBase.Instance.Renderer.GetTexParameter(dimension, GetTextureParameter.TextureMaxLevel, out TexMaxLevel);

                    if (TextureLoaderParameters.Verbose)
                        Trace.WriteLine("Verification: GL: " + ApplicationBase.Instance.Renderer.GetError().ToString() + " TextureMaxLevel: " + TexMaxLevel + ((TexMaxLevel == trueMipMapCount) ? " (Correct.)" : " (Wrong!)"));
#endregion Set States properly
                }

#region Set Texture Parameters
                ApplicationBase.Instance.Renderer.TexParameter(dimension, TextureParameterName.TextureMinFilter, (int)TextureLoaderParameters.MinificationFilter);
                ApplicationBase.Instance.Renderer.TexParameter(dimension, TextureParameterName.TextureMagFilter, (int)TextureLoaderParameters.MagnificationFilter);

                ApplicationBase.Instance.Renderer.TexParameter(dimension, TextureParameterName.TextureWrapS, (int)TextureLoaderParameters.WrapModeS);
                ApplicationBase.Instance.Renderer.TexParameter(dimension, TextureParameterName.TextureWrapT, (int)TextureLoaderParameters.WrapModeT);

                //Game.Instance.Renderer.TexEnv(TextureEnvTarget.TextureEnv, TextureEnvParameter.TextureEnvMode, (int)TextureLoaderParameters.EnvMode);
