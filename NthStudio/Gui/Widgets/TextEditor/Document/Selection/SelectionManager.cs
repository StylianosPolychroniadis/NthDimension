...
namespace NthStudio.Gui.Widgets.TextEditor.Document.Selection
{
	...
	public class SelectionManager : IDisposable
    {
        ...
        /// Creates a new instance of <see cref="SelectionManager"/>
        /// </summary>
        public SelectionManager(IDocument document, TextCanvas pTextCanvas)
        {
            this.document = document;
        ...
        public string SelectedText
        {
            get
            {
                ...
                // OLD_CODE		PriorityQueue queue = new PriorityQueue();

                foreach (ISelection s in selectionCollection)
                {
                    builder.Append(s.SelectedText);
                    // OLD_CODE			queue.Insert(-s.Offset, s);
                }
                // OLD_CODE			
                //				while (queue.Count > 0) {
            ...
            else
            {
                ...
                if (GreaterEqPos(newPosition, SelectionStart)) // selecting forward
                {
                    ...
                    if (selectFrom.where == WhereFrom.Gutter) //&& newPosition.Y != oldPosition.Y)
                        selection.EndPosition = new TextLocation(iTextCanvas.Caret.Column, iTextCanvas.Caret.Line);
                    else
                    {
        ...
        public void ClearSelection()
        {
            ...
            TextLocation newSelectionStart = iTextCanvas.TextView.GetLogicalPosition(mousepos.X - iTextCanvas.TextView.DrawingPosition.X, mousepos.Y - iTextCanvas.TextView.DrawingPosition.Y);
            if (selectFrom.where == WhereFrom.Gutter)
            {
    ...
    {
        public int where = WhereFrom.None;
        // last selection initiator
        public int first = WhereFrom.None;
        // first selection initiator
