...
namespace NthStudio.Gui.Widgets.TextEditor.Document.LineManagement
{
    internal sealed class LineManager
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine("Replace offset=" + offset + " length=" + length + " text.Length=" + text.Length);
        ...
        void RemoveInternal(ref DeferredEventList deferredEventList, int offset, int length)
        {
            ...
            System.Diagnostics.Debug.Assert(charactersRemovedInStartLine > 0);
        ...
        public int GetVisibleLine(int logicalLineNumber)
        {
            ...
            //			System.Diagnostics.Debug.Assert(logicalLineNumber >= foldEnd);
            ...
            for (int i = offset; i < text.Length; i++)
            {
                switch (text[i])
                {
                    case '\r':
						...
						System.Diagnostics.Debug.Assert(false, "Found lone \\r, data consistency problems?");
#endif
                        goto case '\n';
                    case '\n':
                        delimiterSegment.Offset = i;
