namespace NthDimension.FFMpeg
{
    ...
    public class AudioPlayback : Playback<AudioPacket>
    {
        ...
        internal AudioPlayback(AudioBase audio, VideoSource dataSource) : base(dataSource)
        {
            AL = audio;
        ...
        public override void Dispose()
        {
            ...
            AL.GetSource(SourceHandle, AudioBase.ALGetSourcei.BuffersQueued, out int queued);
            if (queued > 0)
            {
        ...
        internal void Update()
        {
            ...
            AL.GetSource(SourceHandle, AudioBase.ALGetSourcei.SourceState, out int state);
            AL.GetSource(SourceHandle, AudioBase.ALGetSourcei.BuffersQueued, out int queued);
            AL.GetSource(SourceHandle, AudioBase.ALGetSourcei.BuffersProcessed, out int processed);
            ...
            {
                var buffer = AudioBuffer.Get(AL, packet.TotalSampleCount, DataSource.AudioChannelCount == 2 ? AudioBase.ALFormat.Stereo16 : AudioBase.ALFormat.Mono16, _sampleRate);
            ...
            if ((AudioBase.ALSourceState)state != AudioBase.ALSourceState.Playing && DataSource.State == PlayState.Playing)
            {
