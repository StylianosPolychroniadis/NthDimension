...
namespace NthDimension.Algebra
{
    ...
    public struct Vector3 : IEquatable<Vector3>
    {
        ...
        public Vector3(Physics.LinearMath.JVector vector)
        {
            X = vector.X;
            Y = vector.Y;
            Z = vector.Z;
        ...
        public float LengthFast
        {
            get
            {
                return 1.0f / MathFunc.InverseSqrtFast(X * X + Y * Y + Z * Z);
            }
        }
        ...
        {
            float scale = 1.0f / this.LengthFast;
            X *= scale;
            Y *= scale;
            Z *= scale;
        }
        ...
        {
            float scale = MathFunc.InverseSqrtFast(X * X + Y * Y + Z * Z);
            X *= scale;
            Y *= scale;
            Z *= scale;
        }
        ...
        {
            float scale = 1.0f / vec.LengthFast;
            vec.X *= scale;
            vec.Y *= scale;
            vec.Z *= scale;
            return vec;
        ...
        {
            float scale = MathFunc.InverseSqrtFast(vec.X * vec.X + vec.Y * vec.Y + vec.Z * vec.Z);
            vec.X *= scale;
            vec.Y *= scale;
            vec.Z *= scale;
            return vec;
        ...
        public static void CalculateAngle(ref Vector3 first, ref Vector3 second, out float result)
        {
            ...
            result = (float)System.Math.Acos(MathFunc.Clamp(temp / (first.LengthFast * second.LengthFast), -1.0, 1.0));
        }
        ...
        {
            string rX = X.ToString(format);
            string rY = Y.ToString(format);
            string rZ = Z.ToString(format);
            return String.Format("({0}{3} {1}{3} {2})", rX, rY, rZ, listSeparator);
        }
