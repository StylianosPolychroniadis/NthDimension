...
namespace RoadGen
{
    public class Segment : ICollidable
    {
        ...
        private static Vector2 ComputeEnd(Vector2 start, float direction, float length)
        {
            ...
            //    return new Vector2(
            //    start.X + length * MathHelper.Sin(direction * MathHelper.Deg2Rad),
            //    start.Y + length * MathHelper.Cos(direction * MathHelper.Deg2Rad)
            //);
            }
        }
            ...
            get
            {
                ...
                if (directionRevision != roadRevision)
                {
                        ...
                        throw new System.NotImplementedException();
                    {
                     ...
                     //   cachedDirection = MathHelper.Sign(direction.X) * MathHelper.Acos(direction.Y / Length) * MathHelper.Rad2Deg;
                    }
                }
        ...
        public float Length
        {
            get
            {
                if (lengthRevision != roadRevision)
                {
                    ...
                    cachedLength = (End - Start).Length;// magnitude;
                }
                return cachedLength;
            }
        }
