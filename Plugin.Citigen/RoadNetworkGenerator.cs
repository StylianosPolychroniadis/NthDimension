...
using System.Linq;
using Plugin.Citigen.Collections;
using Plugin.Citigen.Eppy;
    ...
    public static class RoadNetworkGenerator
    {
        ...
        static bool LocalConstraints(Segment segment, List<Segment> segments, Quadtree quadtree, DebugData debugData)
        {
            ...
            for (int i = 0, j = 0, k = matches.Count - 1; j <= k; i = j += 1)
            {
                    ...
                    if (segment.Intersect(other, out intersection, out t1))
                    {
                        if (t0 == -1 || t1 < t0)
                        {
                            ...
                            action = () =>
                            {
                                ...
                                float minDirectionDiff = Math.Min(dirDiff, Math.Abs(dirDiff - 180.0f));
                ...
                if (priority <= 3)
                {
                    if ((segment.End - other.End).Length <= Config.snapDistance) // (original magnitude).Length
                    {
                        priority = 3;
                        action = () =>
                        {
                ...
                if (priority <= 2)
                {
                    ...
                    throw new System.NotImplementedException();
                    {
                        ...
                        //float lineProj2 = MathHelper.Sign(Vector3.Dot(e0, e1)) * Vector2.SqrMagnitude(proj);
        ...
        static List<Plugin.Citigen.Eppy.Tuple<Segment, int>> GlobalGoals(Segment segment)
        {
            List<Plugin.Citigen.Eppy.Tuple<Segment, int>> newSegments = new List<Plugin.Citigen.Eppy.Tuple<Segment, int>>();
            if (!segment.Severed)
            {
                ...
                if (segment.Highway)
                {
                    ...
                    {
                        newSegments.Add(Plugin.Citigen.Eppy.Tuple.Create(randomStraight, 0));
                        density = randomDensity;
                    }
                    else
                    {
                        newSegments.Add(Plugin.Citigen.Eppy.Tuple.Create(straight, 0));
                    ...
                    if (density > Config.highwayBranchPopulationThreshold)
                    {
                        throw new System.NotImplementedException();
                        {
                            ...
                            //        segment.Highway,
                            //        segment.Severed);
                            //    newSegments.Add(Plugin.Citigen.Eppy.Tuple.Create(leftHighwayBranch, 0));
                            //}
                            ...
                            //        segment.Highway,
                            //        segment.Severed);
                            //    newSegments.Add(Plugin.Citigen.Eppy.Tuple.Create(rightHighwayBranch, 0));
                            //}
                        }
                    }
                }
                ...
                if (straightDensity > Config.streetBranchPopulationThreshold)
                {
                    throw new System.NotImplementedException();
                    {
                        ...
                        //        false,
                        //        false);
                        //    newSegments.Add(Plugin.Citigen.Eppy.Tuple.Create(leftBranch, segment.Highway ? Config.streetBranchDelayFromHighway : 0));
                        //}
                        ...
                        //        false,
                        //        false);
                        //    newSegments.Add(Plugin.Citigen.Eppy.Tuple.Create(rightBranch, segment.Highway ? Config.streetBranchDelayFromHighway : 0));
                        //}
                    }
                }
            }
