...
public class RoadDensityMap : /*MonoBehaviour*/ IMap
{
    ...
    bool WorldToMapCoords(float wX, float wY, out int mX, out int mY)
    {
        throw new System.NotImplementedException();
        {
            ...
            //mX = MathHelper.RoundToInt((w2GX + wX) / dScale);
            //mY = MathHelper.RoundToInt((w2GY + wY) / dScale);
            //return true;
        }
    }
    ...
    void Start()
    {
        ...
        throw new System.NotImplementedException();
        {
            ...
            //bbMinWX = MathHelper.FloorToInt(roadNetwork.BoundingBox.xMin);
            //bbMinWY = MathHelper.FloorToInt(roadNetwork.BoundingBox.yMin);
            //bbMaxWX = MathHelper.CeilToInt(roadNetwork.BoundingBox.xMax);
            //bbMaxWY = MathHelper.CeilToInt(roadNetwork.BoundingBox.yMax);
            //bbWidth = bbMaxWX - bbMinWX;
            //bbHeight = bbMaxWY - bbMinWY;
            //int cWX = bbMinWX + MathHelper.CeilToInt(bbWidth * 0.5f);
            //int cWY = bbMinWY + MathHelper.CeilToInt(bbHeight * 0.5f);

            //dScale = (int)MathHelper.Pow(2, collisionMapDownscale);
            //int gSide = MathHelper.NextPowerOfTwo(MathHelper.CeilToInt(MathHelper.Max(bbWidth, bbHeight))) / dScale + 1;
