...
// Name: composite fragment shader
// Date:
// Rev.:
...
uniform float in_far  = 4000.0; //Z-in_far
...
void main(void)
{
	vec4 selectionTerm = texture(Texture4, v_texture)*(1.0-texture(Texture3, v_texture))*vec4(0.8,0.3,0.8,1.0);
	vec4 sceneTerm  = texture(Texture1, v_texture);
		
	vec4 dofTerm = texture(Texture5, v_texture);
	if(dofTerm.a != 0.0){
			sceneTerm = sceneTerm * (1.0 - dofTerm.a) + dofTerm * dofTerm.a;
	}
